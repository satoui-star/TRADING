...
├─ connexion.py       ← session IBKR (IBKR uniquement)
...
├─ daily.py           ← orchestrateur : capture → IV → QC → forward → validation
...
`daily.py` exécute, dans l'ordre : capture des prix bruts → inversion IV → contrôle qualité → forward par parité → suite de validation. Chaque exécution **ajoute** un instantané daté sans rien écraser.
...
```
python calcul_iv.py     (1) inverse les prix du dernier brut en IV
python qc.py            (2) filtre la chaîne et pose les codes de rejet
python forward.py       (3) reconstruit le forward par parité
python validation.py    (4) note la fiabilité du run (pass/warn/fail)
```

Puis la visualisation (voir §4 et §5).

> **Pourquoi cet ordre ?** `qc.py` lit le dernier `iv_`, `validation.py` lit le dernier `qc_` + le brut correspondant. Si vous lancez `qc` avant `calcul_iv`, il travaillera sur un vieux `iv_`. En cas de doute, relancez les quatre dans l'ordre : c'est rapide et déterministe.
...
|---|---|
| 1 Accès / connexion | `connexion.py` |
...
| 16 Documentation / handover | ce manuel |
